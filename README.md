...
- **style: fix ruff SIM110, E401, I001, UP015, F841 in check_diff.py** (`89ad5436`) — Fixes ruff lint violations in `check_diff.py` diagnostic script (SIM110, E401, I001, UP015, F841).
- **chore: bump version to 2026.07.24.02, add cache-busting, improve type safety and UX** (`86fa417e`) — Bumps plugin version from 2026.07.24.01 to 2026.07.24.02. Adds version query param to main script tag for cache-busting. Adds pre-flight auth check in SettingsPanel to detect stale server with expired cookies before restart. Makes entire server health card clickable to start/fix server with hover/active/focus states. Stops propagation on nested buttons to prevent double-trigger. Coerces all API response fields to strings/arrays for type safety.
- **fix(ci): skip API compat audit when no release tags exist** (`594effa5`) — `audit_public_api_compat.py` called `git describe` to find the latest release tag, but forks without releases fail with "No names found". Now returns `None` from `latest_release_tag()` and gracefully skips the audit with exit code 0 instead of failing with exit code 2.
- **style: fix ruff import order and formatting in sync_firestore_artifact.py** (`1c7aa659`) — Fixes ruff import ordering (alphabetical) and ruff-format function call reformatting in `scripts/sync_firestore_artifact.py`.
